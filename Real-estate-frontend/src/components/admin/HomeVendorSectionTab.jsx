...
export default function HomeVendorSectionTab({ setActiveTab }) {
  const navigate = useNavigate();
  const [title, setTitle] = useState("Team Manager");
  const [subtitle, setSubtitle] = useState("Our Professional Vendors");
  const [btnName, setBtnName] = useState("All Vendors");

  const handleUpdate = (e) => {
    e.preventDefault();
    alert('Vendor section updated successfully!');
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between pb-4 border-b border-slate-100">
        <div>
          <h1 className="text-xl lg:text-2xl font-bold tracking-tight text-slate-900">Vendor Section</h1>
          <div className="flex items-center space-x-2 text-xs text-slate-400 mt-2">
            ...
            <span>&gt;</span>
            <span className="text-slate-500 font-semibold">Vendor Section</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-premium p-6 space-y-6">
        <div className="flex items-center justify-between pb-3 border-b border-slate-55">
          <h3 className="text-sm font-bold text-slate-800">Update Vendor Section</h3>
