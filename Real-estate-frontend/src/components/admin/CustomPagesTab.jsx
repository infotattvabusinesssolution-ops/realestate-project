...
const initialPages = [
  { id: 1, title: 'Privacy Policy', status: 'Active' },
  { id: 2, title: 'Terms & Condition', status: 'Active' }
];

export default function CustomPagesTab({ setActiveTab }) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between pb-4 border-b border-slate-100">
        <div>
          <h1 className="text-xl lg:text-2xl font-bold tracking-tight text-slate-900">Custom Pages</h1>
          <div className="flex items-center space-x-2 text-xs text-slate-400 mt-2">
            <Home size={14} className="text-slate-400 cursor-pointer hover:text-blue-600" onClick={() => navigate('/')} />
            <span>&gt;</span>
            <span className="text-slate-500 font-semibold">Custom Pages</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-premium p-6">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-50 mb-6">
          ...
          <button 
            onClick={() => setActiveTab('custom-pages-add')}
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-slate-100 text-slate-400 font-bold bg-slate-50/50">
                <th className="p-3 w-10">
                  <input 
                    type="checkbox" 
                    checked={selectedIds.length > 0 && selectedIds.length === pages.length}
            ...
            <tbody className="divide-y divide-slate-50">
              ...
              {filteredPages.length === 0 && (
                <tr>
                  <td colSpan="4" className="p-4 text-center text-slate-400">No pages found.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-6 text-xs text-slate-500 font-medium">
          <span>Showing 1 to {filteredPages.length} of {pages.length} entries</span>
