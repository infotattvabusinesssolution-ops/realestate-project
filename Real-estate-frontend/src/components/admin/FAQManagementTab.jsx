  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between pb-4 border-b border-slate-100">
        <div>
          <h1 className="text-xl lg:text-2xl font-bold tracking-tight text-slate-900">FAQ Management</h1>
          <div className="flex items-center space-x-2 text-xs text-slate-400 mt-2">
            <Home size={14} className="text-slate-400 cursor-pointer hover:text-blue-600" onClick={() => navigate('/')} />
            <span>&gt;</span>
            <span className="text-slate-500 font-semibold">FAQ Management</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-premium p-6">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-55 mb-6">
          <div className="flex items-center space-x-4">
            <h3 className="text-sm font-bold text-slate-800">FAQs</h3>
            <select 
              value={lang}
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-slate-100 text-slate-400 font-bold bg-slate-50/50">
                <th className="p-3 w-10">
                  <input 
                    type="checkbox" 
                    checked={selectedIds.length > 0 && selectedIds.length === faqs.length}
            ...
            <tbody className="divide-y divide-slate-50">
              ...
              {filteredFAQs.length === 0 && (
                <tr>
                  <td colSpan="4" className="p-4 text-center text-slate-400">No FAQs found.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-6 text-xs text-slate-500 font-medium">
          <span>Showing 1 to {filteredFAQs.length} of {faqs.length} entries</span>
