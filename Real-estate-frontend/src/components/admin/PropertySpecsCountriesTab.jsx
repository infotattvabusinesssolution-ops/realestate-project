...
export default function PropertySpecsCountriesTab({ setActiveTab }) {
  ...
  const fetchCountries = async () => {
    try {
      ...
      console.error('Error fetching specs countries:', err);
      setLoading(false);
    }
  };
  ...
  const handleSaveCountry = async (e) => {
    ...
    try {
      ...
      alert(err.response?.data?.message || 'Failed to save new country.');
    }
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between pb-4 border-b border-slate-100">
        <div>
          <h1 className="text-xl lg:text-2xl font-bold tracking-tight text-slate-900">Countries</h1>
          <div className="flex items-center space-x-2 text-xs text-slate-400 mt-2">
            ...
            <span>&gt;</span>
            <span className="text-slate-500 font-semibold">Countries</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-premium p-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div className="flex items-center space-x-4">
            <h3 className="text-sm font-bold text-slate-800">Countries</h3>
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-slate-50">
              {filtered.map((country) => (
                <tr key={country.id} className="hover:bg-slate-50/50 transition">
                  <td className="p-3">
                    <input type="checkbox" className="rounded text-blue-600 focus:ring-blue-500" />
                  </td>
                  <td className="p-3 font-bold text-slate-800">{country.name}</td>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-6 text-xs text-slate-500 font-medium">
          <span>Showing 1 to {filtered.length} of {countries.length} entries</span>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
            ...
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <h2 className="text-sm font-bold text-slate-800">Add Country</h2>
              <button 
            ...
            {/* Modal Form */}
            <form onSubmit={handleSaveCountry} className="flex-1 overflow-y-auto p-6 space-y-4 text-xs font-bold text-slate-600 text-left">
              {/* Name */}
              <div className="flex flex-col space-y-1.5">
                <label className="text-slate-600">Country Name*</label>
                <input 
                  required
                  type="text"
              ...
              <div className="flex flex-col space-y-1.5">
                ...
                <span className="text-[10px] text-orange-500 font-bold leading-normal">
                  The higher the serial number is, the later the country will be shown.
                </span>
              </div>
